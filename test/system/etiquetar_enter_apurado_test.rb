...
class EtiquetarEnterApuradoTest < ApplicationSystemTestCase
  setup do
  ...
  test "el pito suena una vez cuando el Enter apurado resuelve, y nunca sin resultados" do
    escuchar("etiquetar:clienteEncontrado")
    demorar("/clientes/buscar", ms: 600)
    ...
    assert_equal 1, eventos.count("etiquetar:clienteEncontrado"),
  ...
  def foco_actual_target
    page.evaluate_script(
      "document.activeElement && (document.activeElement.dataset.etiquetarTarget || '')"
    ).to_s
  end

...
class PagoTest < ActiveSupport::TestCase
  test "requires positive monto" do
    pago = Pago.new(venta: facturas(:pendiente_juan), cliente: clientes(:juan), metodo_pago: "efectivo", monto: 0)
    assert_not pago.valid?
    assert pago.errors[:monto].any?
  end

  test "requires valid metodo_pago" do
    pago = Pago.new(venta: facturas(:pendiente_juan), cliente: clientes(:juan), monto: 10, metodo_pago: "bitcoin")
    assert_not pago.valid?
  ...
  test "accepts efectivo, tarjeta and transferencia" do
    Pago::METODOS.each do |m|
      pago = Pago.new(venta: facturas(:pendiente_juan), cliente: clientes(:juan), monto: 10, metodo_pago: m)
  ...
  test "belongs to venta and cliente" do
    ...
    assert_equal facturas(:pagada_maria), pago.venta
